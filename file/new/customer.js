...
async function sendTicket(){
    ...
    // Mirror a lightweight copy of this order under the signed-in customer's own
    // scope. home.js's "Your order history" reads 'shop-orders' scoped by
    // customer email/name — without this, orders (which are only ever saved
    // scoped by *shop owner* id above) never show up on the customer's home
    // dashboard, which silently falls back to permanent placeholder data.
    const customerScope = S.user?.email || S.user?.name;
    if (customerScope){
        const customerOrders = loadStoredForOwner('shop-orders', [], customerScope);
        customerOrders.push({
            id: order.id,
            ticketNumber: order.ticketNumber,
            item: orderItems.map(i => `${i.qty}× ${i.name}`).join(', '),
            status: order.status,
            total: order.total,
            shopName: S.selectedOwner?.name || 'Shop',
            createdAt: order.createdAt
        });
        saveStoredForOwner('shop-orders', customerOrders, customerScope);
    }
...
function renderCustomer(){
    ...
    <div class="panel ticket">
        ...
        <input type="text" id="customer-name-input" placeholder="Your name (optional)" value="${S.user?.name ? escapeHtml(S.user.name) : ''}">
        ${ticketRows}
