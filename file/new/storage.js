...
function clearStored(options = {}) {
  ...
  if (options.ownerId || options.shopOwnerId || options.scope) {
    ...
    // Keys are built as `${key}:${ownerId}` (see buildScopedStorageKey), so the
    // owner id is a *suffix*, not a prefix. The previous `key.includes(ownerId+':')`
    // check never matched a real key and silently did nothing.
